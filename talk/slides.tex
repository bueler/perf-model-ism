...
\begin{itemize}
\item recall some traditional numerical analysis \dots
\end{itemize}
\begin{minipage}[t][60cm][t]{\textwidth}
\begin{itemize}
\only<1>{
\item we expect \alert{explicit} time stepping for advections
\item for example, forward Euler in 1D gives
	$$\frac{H_j^{\ell+1} - H_j^\ell}{\Delta t} + \frac{\bQ_{j+1/2}^\ell - \bQ_{j-1/2}^\ell}{\Delta x} = a_j^\ell$$

    \begin{itemize}
    \item[$\circ$] where $\bQ = \bU H$
...
\item for example, backward Euler in 1D gives
