...
\bigskip
\begin{itemize}
\item recall traditional numerical analysis applied to these equations
\end{itemize}
...
\only<1>{\item[]}
\only<2>{
\item we expect \alert{explicit} time stepping for advections
\item for example, forward Euler:
	$$\frac{H_j^{\ell+1} - H_j^\ell}{\Delta t} + \frac{(\bU H)_{j+1/2}^\ell - (\bU H)_{j-1/2}^\ell}{\Delta x} = a_j^\ell$$

    \begin{itemize}
    \item[$\circ$] need flux approximations: upwinding, Lax-Wendroff, streamline diffusion, flux-limiters, \dots
    \item[$\circ$] conditional stability
    \item[$\circ$] maximum time step from CFL using $\max |\bU|$
    \end{itemize}
}
\only<3>{
\item we expect \alert{implicit} time stepping for diffusions
\item for example, backward Euler:
	$$\frac{H_j^{\ell+1} - H_j^\ell}{\Delta t} - \frac{D_{j+\frac{1}{2}} (s_{j+1}^{\ell+1} + s_{j}^{\ell+1}) - D_{j-\frac{1}{2}} (s_{j}^{\ell+1} + s_{j-1}^{\ell+1})}{2 \Delta x} = a_j^\ell$$

    \begin{itemize}
    \item[$\circ$] or: Crank-Nicolson, BDF, \dots
    \item[$\circ$] unconditional stability
    \item[$\circ$] must solve equations at each step
    \end{itemize}
}
\end{itemize}
\end{minipage}
...
\item current-technology models mostly use explicit time stepping
    \begin{itemize}
    \item[$\circ$] embarrassing: the problem is a DAE
    \item[$\circ$] performance and design consequences: stability is determined by the coupled solution velocity $\bU$
    \end{itemize}

\medskip
\item<2> \alert{implicit time-stepping} is expected for DAE problems
    \begin{itemize}
    \item[$\circ$] the obvious alternative
    \end{itemize}
\item<2> a sequence of problems (``equations'') must be solved at each time step: NCP coupled to Stokes
\end{itemize}
\end{frame}
...
\item $m$ = (\# of surface elevation, velocity, and pressure unknowns)
\item domain $\Omega \subset \RR^2$ (map-plane) has characteristic width $L$
\item map-plane (2D) mesh cells have characteristic diameter $h$:
  $$m \sim \frac{L^2}{h^2}  \quad \iff \quad h \sim \frac{L}{m^{1/2}}$$
\item recall mass-continuity equation schemata:
{\footnotesize
\begin{align*}
\text{advective schema:} && \frac{\partial H}{\partial t} + \nabla \cdot \left(\bU H\right) &= a \\
\text{diffusion schema:} && \frac{\partial H}{\partial t} - \nabla \cdot \left(D \grad s\right) &= a
...
\begin{frame}{ice sheet model performance}
...
\begin{itemize}
\item main point: \alert{explicit time-stepping stability restrictions imply many stress-balance solves}
    \begin{itemize}
    \item[$\circ$] $\alpha$ for stress balance solver important
    \item[$\circ$] however, even optimality ($\alpha=0$) cannot fix low model performance
    \end{itemize}
\item switch to \alert{implicit time-stepping} for unconditional stability?
    ...
    \item[$\circ$] each step is a NCP-coupled-to-Stokes problem
        \begin{itemize}
        \item[$\vartriangleright$] or SIA NCP
        \item[$\vartriangleright$] in any case, a \alert{free-boundary problem}
        \item[$\vartriangleright$] next slide parameterizes expense of these coupled, implicit-step solves as $O(m^{1+\beta})$; optimal is $\beta=0$
        \end{itemize}
...
\item<2> \alert{existing} implicit time-stepping ice sheet models:
    ...
    \item[$\circ$] no other fully-implicit NCP solvers known?
    \end{itemize}
\end{itemize}
\end{frame}


\begin{frame}{ice sheet model performance 2}
...
\begin{center}
\includegraphics[width=0.45\textwidth]{images/cnn-jouvet.png}

\includegraphics[width=0.65\textwidth]{images/tiles-jouvet.png}
\end{center}
\end{frame}
