...
\begin{itemize}
\item from now on assume our ice sheet model is \emph{not} shallow
\item whether time steps are explicit or implicit, a Stokes problem is solved at each step
\item for a given icy domain $\Lambda \subset \RR^3$:
\begin{align*}
...
\item<3-> \alert{coming soon} from current research:
   ...
   \end{enumerate}
\item<4-> scalable solvers for the implicit-step problem require advances in \alert{multilevel solutions of non-local variational inequalities}
\end{itemize}
\end{frame}
