...
\begin{itemize}
\item a thickness-transport form exposes the basic ice sheet model
\item define:
\begin{align*}
...
\item[] \emph{answer:} not really, because ice flows (mostly) downhill
\item an alternative form is the \emph{shallow diffusion} equation for ice sheets:
...
\begin{itemize}
\item the model on the last slide is \emph{not yet} in common use for ice sheet simulations
    \begin{itemize}
    \item[$\circ$] for now, it is too expensive at scale
    \item[$\circ$] one goal: \alert{make it fast enough to use!}
    \end{itemize}
