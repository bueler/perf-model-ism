...
\begin{itemize}
\item nonlinear complementarity problem (NCP) \onslide<2->{coupled to Stokes}:
\begin{align*}
...
{\footnotesize
\begin{align*}
\text{advective ice sheet model schema:} && \frac{\partial H}{\partial t} + \nabla \cdot \left(\bU H\right) &= a \\
\text{diffusion ice sheet model schema:} && \frac{\partial H}{\partial t} - \nabla \cdot \left(D \grad s\right) &= a
\end{align*}
}
...
\item for example, backward Euler:
	$$\frac{H_j^{\ell+1} - H_j^\ell}{\Delta t} - \frac{D_{j+\frac{1}{2}} (H_{j+1}^{\ell+1} + H_{j}^{\ell+1}) - D_{j-\frac{1}{2}} (H_{j}^{\ell+1} + H_{j-1}^{\ell+1})}{2 \Delta x} = a_j^\ell$$

    \begin{itemize}
    \item[$\circ$] or use: Crank-Nicolson, BDF, \dots
    \end{itemize}
}
\end{itemize}
\end{minipage}
