% generate complexity data for bueler 2016 and isaac et al 2015 and tumarino et al 2016
...
% ISAAC:  publication uses kitsi.*nitsi (last), but probably 1st is correct
ni = [7e6 51e6 383e6];
kitsi = [66 75 90]; % total ("overall") Krylov iterations; averaged over P vals
pi = polyfit(log(ni),log(kitsi),1)
%ans = 0.0775   2.9589   ==>  alpha = 0.0775
%flopsi = ni .* kitsi;
%pflopsi = polyfit(log(ni),log(flopsi),1)
%ans = 1.0775   2.9589  ==>  alpha = 0.0775  (same as above)
%nitsi = [7 10 11]; % round down average Newton iterations to integer
%pi = polyfit(log(ni),log(kitsi .* nitsi),1)
