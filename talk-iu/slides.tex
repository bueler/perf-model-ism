...
\begin{column}{0.6\textwidth}
\begin{itemize}
\item for simplicity/clarity of the upcoming modeling, I will ignore parts of glacier physics in the remainder of my talk
...
\medskip
\item {\footnotesize see UAF's \href{https://pism.io/}{Parallel Ice Sheet Model (\texttt{pism.io})}, for example, which includes these and other processes}
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\hfill \includegraphics[width=0.9\textwidth]{../images/map-glacier-ice.png}
...
\end{align*}
    ...
    \begin{itemize}
    \item<2>[$\circ$] but in $\infty$ dimensions \dots and actually subject to complementarity
    \end{itemize}
\end{itemize}
\end{frame}
...
\medskip
\item \dots and ice sheet geometry evolution is addressed with minimal awareness of complementarity

\medskip
\item the NCP-coupled-to-Stokes basic model is \emph{not yet} in common use for high-resolution, long-duration ice sheet simulations
    \begin{itemize}
...
\section{numerics: time-stepping and solver scaling}
...
\item<2> observation: the NCP-coupled-to-Stokes system has no characteristic curves
\end{itemize}
\end{frame}
...
\item the diffusion schema is literal in the shallow ice approximation
    ...
    \end{itemize}
\item the fact that ice flows downhill has \emph{time-stepping stability} consequences for the basic model
    \begin{itemize}
    \item[$\circ$] regardless of your above schema preference!
    \end{itemize}
...
\begin{itemize}
\item as mentioned, the simplest of glacier flow approximations is the ``lubrication'' approximation: \alert{shallow ice approximation} (SIA)
...
\item $\Phi(s)$ resolves to a \alert{doubly-nonlinear differential operator}
    ...
    \item[$\circ$] well-posedness for this problem as weak form \alert{variational inequality} (Calvo et al 2002, Jouvet \& Bueler 2012, Piersanti \& Temam 2022)
    \end{itemize}
\end{itemize}
\end{frame}
...
\item current-technology large-scale models use explicit time stepping
    ...
    \end{itemize}
\item yet most researchers believe the advection schema
    \begin{itemize}
